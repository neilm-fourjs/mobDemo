...
FUNCTION init_debug( l_logDir STRING, l_logFile STRING )
	IF l_logDir.trim() IS NULL THEN LET l_logDir = fgl_getResource("debug.logpath") END IF
	IF l_logDir.trim() IS NULL THEN LET l_logDir = "." END IF
	IF l_logFile IS NULL THEN LET l_logFile = base.Application.getProgramName()||".log" END IF
	IF NOT os.path.exists(l_logDir) THEN
		IF NOT os.path.mkdir(l_logDir) THEN
			DISPLAY SFMT("Failed to make logpath '%1'", l_logDir)
			LET l_logDir = "."
		END IF
	END IF
